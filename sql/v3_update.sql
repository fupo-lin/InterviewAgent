...
SET @sql = (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE interview_evaluations ADD COLUMN agent_run_id BIGINT NULL AFTER session_id',
    'SELECT 1'
  )
...
SET @sql = (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE interview_evaluations ADD COLUMN schema_version VARCHAR(80) NOT NULL DEFAULT ''InterviewEvaluation.v1'' AFTER agent_run_id',
    'SELECT 1'
  )
  FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA = @schema_name
...
SET @sql = (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE interview_evaluations ADD COLUMN evidence_refs JSON NULL AFTER schema_version',
    'SELECT 1'
  )
  FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA = @schema_name
...
SET @sql = (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE interview_evaluations ADD CONSTRAINT fk_interview_evaluations_agent_run FOREIGN KEY (agent_run_id) REFERENCES agent_runs(id) ON DELETE SET NULL',
    'SELECT 1'
  )
